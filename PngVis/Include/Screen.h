...
public:
    ...
    void Display();
    void Clear(Color color = Color(255, 255, 255));
...
void Screen::Display()
{
    ...
    {
        if (screen[i].color != prevColor)
        {
            buffer << "\e[48;2;" << screen[i].color.r << ';' << screen[i].color.g  << ';' << screen[i].color.b << "m"; // Set color only when it changes
            prevColor = screen[i].color;
        }
